"""
Test RF-CPSS auf Erlotinib, Cisplatin, Dabrafenib
Paper-conform implementation mit method='rf'
"""
...
def test_drug(drug_name, expected_markers, B=60):
    """Test RF-CPSS on one drug with biomarker validation"""
    
    print("="*90)
    print(f"DRUG: {drug_name} (RF-CPSS, paper-conform)")
    print("="*90)
    ...
    X = pd.read_parquet(data_dir / 'X_general_sis_reduced.parquet')  # RF: General SIS (all features)
    ...
    # Run RF-CPSS with paper-conform implementation
    print(f"\nRunning paper-conform RF-CPSS with B={B} pairs...")
    selection_probs, feature_names = cpss_with_fixed_q(
        X, y, B=B, q=q, method='rf', random_state=42
    )
    ...
    results.to_csv(f"cpss_rf_{drug_name}_B{B}_results.csv", index=False)
...
if __name__ == "__main__":
    print("\n" + "="*90)
    print("RF-CPSS TEST: Paper-conform implementation with Random Forest")
    print("="*90)
    print("\nChanges from old implementation:")
    print("  1. q is FIXED (not adaptive)")
    print("  2. RF selects exactly q features on each subsample")
    print("  3. Permutation importance + feature-type weighting")
    print("  4. NO invalid unimodality tests")
    print("  5. Only worst-case bound (always valid)")
    ...
    for drug, markers in [
        ...
        strong, weak, total = test_drug(drug, markers, B=60)
